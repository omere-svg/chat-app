// The tutor answers strictly from retrieved context. The hard grounding guarantee is
// enforced in code (no chunks -> no LLM call -> fixed refusal); this prompt keeps the
// model from straying beyond the supplied chunks when chunks DO exist.
...
// Streamed verbatim when retrieval returns nothing above the relevance threshold. No
// LLM is called in that case, so the answer cannot hallucinate.
